                                   ...
                                   size_t *pnread)
{
  ...
  *pnread = 0;
  if(cf) {
    ...
    nread = Curl_conn_cf_recv(cf->next, data, (char *)buf, buflen, &result);
                                       ...
                                       size_t *pwritten)
{
  ...
  if(cf) {
    ...
    CURL_TRC_CF(data, cf, "[0] nw_out_writer(len=%zu) -> %zd, %d",
                buflen, nwritten, result);
    *pwritten = (nwritten < 0) ? 0 : (size_t)nwritten;
    return result;
  }
  else {
    *pwritten = 0;
  ...
  *pnread = 0;
  if(!Curl_bufq_is_empty(&ctx->tunnel.recvbuf)) {
    ...
    if(ctx->tunnel.closed) {
      result = h2_handle_tunnel_close(cf, data);
    }
    else if(ctx->tunnel.reset ||
                                ...
                                CURLcode *err)
{
  ...
  if(ctx->tunnel.closed) {
    ...
    *err = CURLE_SEND_ERROR;
    ...
    size_t n;
    *err = Curl_bufq_write(&ctx->tunnel.sendbuf, buf, len, &n);
